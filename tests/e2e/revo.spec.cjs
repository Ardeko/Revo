...
test('local assets, invite, chat, audio settings and keyboard navigation', async ({ page }, testInfo) => {
    ...
    for (const path of ['/js/room.js', '/lib/signalr/signalr.min.js', '/js/noise-suppressor/rnnoise.wasm']) {
...
test('RNNoise load failure falls back to browser processing without losing chat', async ({ page }) => {
    const errors = await observeMedia(page);
    await page.route('**/rnnoise*.wasm', route => route.abort());
    await enter(page, 'Yedek');
    await page.locator('#settingsButton').click();
    await expect(page.locator('#audioProcessingStatus')).toContainText('Tarayıcı');
    await page.keyboard.press('Escape');
    await page.locator('#messageInput').fill('Yedek ses işleme etkin.');
    await page.locator('#messageForm button[type=submit]').click();
    await expect(page.locator('#messagesList')).toContainText('Yedek ses işleme etkin.');
    expect(errors).toEqual([]);
});
